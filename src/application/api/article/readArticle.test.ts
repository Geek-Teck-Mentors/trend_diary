...
import authTestHelper from '@/test/helper/authTestHelper'
...
describe('POST /api/articles/:article_id/read', () => {
  ...
  async function setupTestData(): Promise<void> {
    // アカウント作成・ログイン
    const userData = await authTestHelper.create('test@example.com', 'password123')
    const loginData = await authTestHelper.login('test@example.com', 'password123')
    testActiveUserId = userData.userId
  ...
  async function requestReadArticle(articleId: string, accessToken: string, readAt?: string) {
