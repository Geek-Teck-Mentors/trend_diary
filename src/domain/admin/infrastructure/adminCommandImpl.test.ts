  ...
  describe('grantAdminRole', () => {
    describe('基本動作', () => {
      it('Admin権限を正常に付与できる', async () => {
        ...
        mockDb.user.findUnique.mockResolvedValue(mockActiveUser)
        ...
        expectDatabaseCalls({
          user: { where: { userId: testData.userId } },
      ...
      it('異なるuserIdで複数のAdmin権限を付与できる', async () => {
        ...
        const mockActiveUser2 = createMockUser({
          userId: userId2,
        ...
        mockDb.user.findUnique.mockResolvedValueOnce(createMockUser())
        mockDb.adminUser.findUnique.mockResolvedValueOnce(null)
        mockDb.adminUser.create.mockResolvedValueOnce(createMockAdminUser())

        mockDb.user.findUnique.mockResolvedValueOnce(mockActiveUser2)
        mockDb.adminUser.findUnique.mockResolvedValueOnce(null)
    ...
    describe('例外・制約違反', () => {
      it('user検索時のデータベースエラーを適切にエラーを返す', async () => {
        setupDatabaseError(mockDb.user.findUnique)
