...
describe('AdminQueryServiceImpl', () => {
  let service: AdminQueryServiceImpl
  const activeUserId = 123456789n

  beforeEach(() => {
    vi.clearAllMocks()
    service = new AdminQueryServiceImpl(mockDb)
  })
  ...
  describe('findAdminByActiveUserId', () => {
    describe('基本動作', () => {
      ...
      it('異なるActiveUserIdで複数のAdmin情報を検索できる', async () => {
        ...
        const [result1, result2] = await Promise.all([
          service.findAdminByActiveUserId(activeUserId),
          service.findAdminByActiveUserId(activeUserId2),
        ])
    ...
    describe('境界値・特殊値', () => {
      it('存在しないActiveUserIdの場合nullを返す', async () => {
        ...
        const result = await service.findAdminByActiveUserId(nonExistentActiveUserId)
  ...
  describe('findAllUsers', () => {
    describe('基本動作', () => {
      ...
      it('検索クエリでユーザーをフィルタリングできる', async () => {
        ...
        const result = await service.findAllUsers(searchQuery)
      ...
      it('ページネーションが正しく動作する', async () => {
        ...
        const result = await service.findAllUsers(paginationQuery)
    ...
    describe('境界値・特殊値', () => {
      it('大きなページ番号でも正常に処理できる', async () => {
        ...
        const result = await service.findAllUsers(largePageQuery)
      ...
      it('大きなlimit値でも正常に処理できる', async () => {
        ...
        const result = await service.findAllUsers(largeLimitQuery)
      ...
      it('特殊文字を含む検索クエリでも正常に処理できる', async () => {
        ...
        const result = await service.findAllUsers(specialCharQuery)
      ...
      it('日本語の検索クエリでも正常に処理できる', async () => {
        ...
        const result = await service.findAllUsers(japaneseQuery)
